...
$view1 = '';
if ($status_ready) {
    $view1 .= '<table class="table">';
    $view1 .= '<thead><tr><th>タイトル</th><th>作成日時</th></tr></thead>';
    $view1 .= '<tbody>';
...
if ($status_ready) {
    ...
    $view2 .= '<thead><tr><th>お題</th><th>コメント</th><th>作成日時</th></tr></thead>';
    $view2 .= '<tbody>';
    while ($r = $stmt_ready->fetch(PDO::FETCH_ASSOC)) {
        $view2 .= '<tr>';
        // $view2 .= '<td>' . $r["id"] . '</td>';
        // $view2 .= '<td><a href="detail1_text.php?id=' . $r["id"] . '">' . h($r['text_prompt']) . '</a></td>';
        $view2 .= '<td><a href="detail1_response.php?id=' . $r["id"] . '">' . h($r['generated_topic']) . '</a></td>';
        
        $view2 .= '<td><a href="detail1_text.php?id=' . $r["id"] . '">' . 'コメント' . '</a></td>';
        // $view2 .= '<td><a href="detail1_response.php?id=' . $r["id"] . '">' . 'テキスト' . '</a></td>';
